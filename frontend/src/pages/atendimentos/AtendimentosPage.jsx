...
// ... (O código dos componentes AgendamentoModal e DiaDetalhesModal continua o mesmo, sem alterações)
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        ...
                        <select value={clienteId} onChange={e => setClienteId(e.target.value)} required>
                            ...
                            {clientes.map(c => <option key={c.id} value={c.id}>{c.NOMECLIENTE}</option>)}
                        </select>
                    </div>
                    <div className="form-group">
                        <label>Advogado</label>
                        <select value={advogadoId} onChange={e => setAdvogadoId(e.target.value)} required>
                     ...
                     <div className="form-group">
                        ...
                        <textarea value={observacao} onChange={e => setObservacao(e.target.value)} placeholder="Adicione uma breve observação..."></textarea>
                    </div>
        ...
        <div className="modal-overlay">
            <div className="modal-content">
                        ...
                        atendimentosDoDia.sort((a,b) => a.horario.localeCompare(b.horario)).map(at => (
                            <div key={at.id} className={`atendimento-item status-${at.status.toLowerCase()}`}>
                                <div className="atendimento-info">
                                    <strong>{at.horario}</strong> - {at.clienteNome}
                                    <small>com Dr(a). {at.advogadoNome}</small>
                                    {at.observacao && <p className="atendimento-obs">Obs: {at.observacao}</p>}
                                </div>
                                <div className="atendimento-actions">
                                    {at.status === 'Agendado' && (
                                         <button onClick={() => onStatusChange(at.id, 'Concluído')} className="btn-concluir-dia">
                                             <i className="fa-solid fa-check"></i> Concluído
                                         </button>
                                    )}
                                     {at.status === 'Concluído' && (
                                         <button onClick={() => onStatusChange(at.id, 'Agendado')} className="btn-reagendar-dia">
                                             <i className="fa-solid fa-arrow-rotate-left"></i> Reagendar
                                         </button>
                                    )}
                                </div>
                            </div>
                ...
                <div className="modal-footer">
                    <button onClick={onAddNovo} className="btn-add-novo">
                        <i className="fa-solid fa-plus"></i> Adicionar Novo Atendimento
                    </button>
                </div>
            </div>
        </div>
...
function AtendimentosPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Busca apenas clientes da localização selecionada
            ...
            // Busca apenas atendimentos da localização selecionada
    ...
    const handleSaveAgendamento = async (data) => {
        try {
            ...
            await addDoc(collection(db, "agendamentos"), {
                ...
                clienteNome: clienteSelecionado.NOMECLIENTE,
                advogadoId: data.advogadoId,
                advogadoNome: advogadoSelecionado.nome,
                observacao: data.observacao,
                status: 'Agendado',
                location: clienteSelecionado.LOCATION, // <-- CORREÇÃO: Salva a localização
            });
    ...
    // ... (O restante do código do componente continua o mesmo)
    ...
    const handleStatusChange = async (id, status) => { const atendimentoRef = doc(db, "agendamentos", id); try { await updateDoc(atendimentoRef, { status }); Swal.fire('Sucesso!', `Atendimento atualizado para ${status}.`, 'success'); setIsDetalhesModalOpen(false); fetchData(); } catch(error) { console.error("Erro ao mudar status: ", error); Swal.fire('Erro!', 'Não foi possível atualizar.', 'error'); }};
